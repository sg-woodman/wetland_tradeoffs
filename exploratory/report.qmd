...
2.  We also expect aquatic productivity to scale with biodiversity but to have a mixed relationship with GHG fluxes because CO<sub>2</sub> should be negatively correlated with NEP due to more autotrophic production while higher NEP should increase CH<sub>4</sub> as the availability of substrate for methanogenous increase.

3.  Finally, we hypothesis an inverse connection between emergent vegetation extent and NEP.
...
((df %>% 
  ...
  scale_y_continuous(name = "Total biomass (Mg)", 
                     limits = c(0, 30)) + 
   geom_signif(
    comparisons = list(c("Cropland", "Grassland")),
...
```{r}
lc_biomass_mod <- lm(total_mass_Mg ~ land_class, data = df[df$land_class != "Both",])
summary(lc_biomass_mod)

lc_area_mod <- lm(full_wetland_area_m2 ~ land_class, data = df[df$land_class != "Both",])
summary(lc_area_mod)

lc_density_mod <- lm(total_density ~ land_class, data = df[df$land_class != "Both",])
summary(lc_density_mod)
```
...
df %>% 
  filter(land_class != "Both") %>% 
  ggplot(aes(x = latitude_y, y = total_mass_Mg, 
             colour = land_class)) + 
  geom_point() + 
  scale_colour_manual(name = "Land Use",
                      values = c("#4fad3c","#f4be4a")) + 
  scale_y_continuous(name = "Total biomass (Mg)") + 
  scale_x_reverse(name = "Latitude (°)", 
                     limits = c(53.2, 50.5)) + 
  theme_classic2() + 
...
df %>% 
  filter(land_class != "Both") %>% 
  ggplot(aes(x = latitude_y, y = full_wetland_area_m2, 
             colour = land_class)) + 
  geom_point() + 
  scale_colour_manual(name = "Land Use",
                      values = c("#4fad3c","#f4be4a")) + 
...
(df %>% 
  ...
  (df %>% 
  filter(land_class != "Both") %>% 
  ggplot(aes(x = land_class, y = aerial_gpp - aerial_r, 
             fill = land_class)) + 
  geom_boxplot() + 
  scale_fill_manual(values = c("#4fad3c","#f4be4a")) + 
  ...
  geom_boxplot() + 
  scale_fill_manual(values = c("#4fad3c","#f4be4a")) + 
  scale_y_continuous(name = "GPP:R<sub>aq</sub>", 
                     limits = c(0.2, 1.2)) + 
    geom_signif(
    comparisons = list(c("Cropland", "Grassland")),
    map_signif_level = TRUE,
...
```{r}
lc_gpp_mod <- lm(aerial_gpp ~ land_class, data = df[df$land_class != "Both",])
summary(lc_gpp_mod)

lc_r_mod <- lm(aerial_r ~ land_class, data = df[df$land_class != "Both",])
summary(lc_r_mod)

lc_nep_mod <- lm(I(aerial_gpp - aerial_r) ~ land_class, data = df[df$land_class != "Both",])
summary(lc_nep_mod)

lc_gppr_mod <- lm(I(aerial_gpp/aerial_r) ~ land_class, data = df[df$land_class != "Both",])
summary(lc_gppr_mod)
```
